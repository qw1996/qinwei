...
<%@taglib prefix="qw" uri="http://github.com/zhangkaitao/tags/zhang-functions" %>
...
<table class="table">
    ...
    <tbody>
        <c:forEach items="${sessions}" var="session">
            <tr>
                <td>${session.id}</td>
                <td>${qw:principal(session)}</td>
                <td>${session.host}</td>
                <td><fmt:formatDate value="${session.lastAccessTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                <td>${qw:isForceLogout(session) ? '是' : '否'}</td>
                <td>
                    <c:if test="${not qw:isForceLogout(session)}">
